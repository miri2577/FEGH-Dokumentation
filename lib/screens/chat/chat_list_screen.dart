...
class _ChatListScreenState extends State<ChatListScreen> {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'Server: ${MatrixChatService.defaultHomeserver}',
    ...
    if (mounted) {
      ...
      if (!ok) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Anmeldung fehlgeschlagen'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Future<void> _register() async {
    ...
    if (mounted) {
      ...
      if (ok) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Registriert! Sie sind angemeldet.'), backgroundColor: Colors.green),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Registrierung fehlgeschlagen'), backgroundColor: Colors.red),
        );
      }
    }
  }
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: '@user:cavia-aperea.de oder Team-Name',
                border: OutlineInputBorder(),
              ),
            ),
          ],
